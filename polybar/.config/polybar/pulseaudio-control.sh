...
TEXT_COLOR="%{F#dfdfdf}" # Color when the audio is not muted
ICON_COLOR="%{F#73d216}" # Color of the icons
NOTIFICATIONS="yes"  # Notifications when switching sinks if enabled
AUDIO_PLAY="yes" # Whether to play a sound on volume change or not
AUDIO_CHANGE_SOUND=~/.config/polybar/volume.aiff # The sound to play when changing volume
MSG_ID="9910491" # Arbitrary but unique message id
...
# Needs "sox" to be installed
function playSound() {
    play -q "${AUDIO_CHANGE_SOUND}"
}
...
function volUp() {
    ...
    if [ $NOTIFICATIONS = "yes" ]; then showNotification "$curSink"; fi
    if [ $AUDIO_PLAY = "yes" ]; then playSound; fi
}
...
function showNotification() {
    ...
    if [ "$isMuted" = "yes" ]; then
        $notify --appname "PulseAudio Control" --urgency low --icon audio-volume-muted \
            "Volume: muted" &
    else
        $notify --appname "PulseAudio Control" --urgency low --icon audio-volume-high \
...
function output() {
    ...
    if [ "$isMuted" = "yes" ]; then
        echo "${ICON_COLOR}${MUTED_ICON}${MUTED_COLOR}${curVol}%${END_COLOR}"
    else
        echo "${ICON_COLOR}${volIcon}${TEXT_COLOR}${curVol}%${END_COLOR}"
    fi
}
